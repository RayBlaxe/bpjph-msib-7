...
const Component = ({ type, ...props }) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        const url =
          `https://prod-api.halal.go.id/v1/referensi/data_lhln?nama_lhln=${search}&page=${page}&size=${sizePerPage}` +
          searchStatus;
        const options = {
          headers: {
    ...
    <React.Fragment>
      <div className="container mt-4">
        <form className="mb-4">
          <div className="grid grid-flow-row-dense gap-2 sm:grid-cols-1 md:grid-cols-5 lg:grid-cols-5">
            ...
            <select
              ...
              className={`bg-white-50 dark:bg-white-700 my-1 block w-full w-auto rounded-md border border border-2 border-purple border-purple p-2.5 text-sm text-gray-900 placeholder-gray-400 focus:rounded-md md:my-0`}
            >
              <option value="">All</option>
              <option value=" MoU">MoU</option>
              <option value=" MRA">MRA</option>
              <option value="ASSESMENT PROCESS - CERTIFICATE ISSUED">
                ASSESMENT PROCESS - CERTIFICATE ISSUED
              </option>
        ...
        <ResultText
          ...
          page={page}
          sizePerPage={20}
          total={response?.data?.total_items || 1}
        />
        ...
        <div className="my-5 flex flex-col items-center justify-between">
          <div className="xs:mt-0 mt-2 inline-flex">
            {page > 1 ? (
              <PrevButton onClick={() => setPage(page - 1)} />
            ) : (
              <div />
            )}
            {response?.data?.total_pages > page ? (
